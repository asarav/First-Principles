...
func run(t) -> void:
	...
	t.case("step advances a multi-wave circuit")
	var staged := Machine.new()
	staged.add_component(ComponentTypes.create(ComponentTypes.INPUT, "in_a"))
	staged.add_component(ComponentTypes.create(ComponentTypes.INPUT, "in_b"))
	staged.add_component(ComponentTypes.create(ComponentTypes.AND, "and_1"))
	staged.add_component(ComponentTypes.create(ComponentTypes.OUTPUT, "out_y"))
	staged.add_connection(SimConnection.new("in_a", "out", "and_1", "a"))
	staged.add_connection(SimConnection.new("in_b", "out", "and_1", "b"))
	staged.add_connection(SimConnection.new("and_1", "out", "out_y", "in"))
	staged.set_input("in_a", SignalValue.from_bool(true))
	staged.set_input("in_b", SignalValue.from_bool(true))
	t.is_true(staged.step())
	t.is_true(staged.get_output("out_y").to_variant() == null)
	t.is_true(staged.step())
	t.is_true(staged.get_output("out_y").equals(SignalValue.from_bool(true)))
	t.is_false(staged.step())
